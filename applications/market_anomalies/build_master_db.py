...
def build_master_db(wrds_username: str):
    """
    Build a CRSP security master mapping:

      permno ↔ permco ↔ ticker ↔ cusip ↔ name date range

    and save as applications/data/wrds/master_db.parquet
    """
    ...
    q = """
        ...
        WHERE names.exchcd IN (1 \
            , 2 \
            , 3)
          AND names.shrcd IN (10 \
            , 11)
    ...
    project_root = Path(__file__).resolve().parents[2]   # .../Agentics
